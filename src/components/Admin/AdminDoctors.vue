...
export default {
  ...
  methods: {
    edit (ssn) {
      this.$router.push('/admin/doctors/edit/' + ssn)
    },
    deleteD (ssn) {
    ...
    getDD () {
      axios.get('http://localhost:5000/admin/doctors', {headers: {'x-access-token': localStorage.adminToken}}).then((res) => {
        if (res.status !== 200) {
          console.log(res.data.message)
        } else {
