  ...
  <main class="page-content">
    <div class="container">
      <form class="form-signin" @submit.prevent="update">
        <label for="no" class="sr-only">Room number</label>
        <select v-model="roomno" class="form-control" style="height:100%" required autofocus>
            <option @click="edit(room[0])" v-for="room in rooms" :key="room[0]" :value="room[0]">{{room[0]}}</option>
        </select>
        <button class="btn btn-lg btn-primary btn-block" type="submit">Assign Patient to room</button>
      </form>
    </div>
...
export default {
  ...
  methods: {
    update () {
      axios.post('http://localhost:5000/admin/PtR', {'ssn': this.ssn, 'roomno': this.roomno}, {headers: {'x-access-token': localStorage.adminToken}})
        .then((request) => {
    ...
    async getData () {
      await axios.get('http://localhost:5000/admin/patient/get/' + this.ssn, {headers: {'x-access-token': localStorage.adminToken}}).then((res) => {
        this.name = res.data.data[0]
        this.phone = res.data.data[1]
        this.email = res.data.data[2]
      })
      await axios.get('http://localhost:5000/admin/PtR/get', {headers: {'x-access-token': localStorage.adminToken}}).then((res) => {
